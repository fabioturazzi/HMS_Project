...
public class RoomSearchBookingController {
	...
	public String applySearchRoomTypes(HttpSession session, Model model,
		...
		//Initial filter check, verify if room capacity and room type match search specifications
		...
		if (searchedRoomType.getAmenities().length > 0
			...
			//Iterate through amenities on the filter to verify which rooms satisfy the constraints
		...
		for (int i = 0; i < secondFilterCheck.size(); i++) {
			...
			totalCost.add(secondFilterCheck.get(i).getDailyPrice() * ChronoUnit.DAYS.between(LocalDate.parse(searchedRoomType.getStartDateFormControl()), LocalDate.parse(searchedRoomType.getEndDateFormControl())));
		}
		...
		if(secondFilterCheck.isEmpty()) {
			model.addAttribute("message", "No rooms with the searched specifications are available for these dates.");
		} else {
			SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
			Date date = new Date(System.currentTimeMillis());
			
			model.addAttribute("message", "Search filter updated " + formatter.format(date));
		}
